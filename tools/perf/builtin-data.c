...
static int data_cmd_index(int argc, const char **argv);
...
static int __data_cmd_index(struct data_index *idx)
{
	...
	if (!output_name) {
		if (asprintf(&output, "%s.out", session->data->file.path) < 0) {
			pr_err("memory allocation failed\n");
			return -1;
		}
...
int data_cmd_index(int argc, const char **argv)
{
	bool force = false;
	struct perf_session *session;
	...
	idx.tool.machines = machines__new();
	if (idx.tool.machines == NULL)
		return -1;

	data.file.path = input_name;
	data.force = force;
	session = perf_session__new(&data, false, &idx.tool);
	if (session == NULL)
		goto out;
	...
	ret = __data_cmd_index(&idx);

	perf_session__delete(session);
out:
	machines__delete(idx.tool.machines);
	return ret;
}

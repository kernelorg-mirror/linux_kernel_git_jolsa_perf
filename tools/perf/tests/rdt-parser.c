...
static int check_rdt_resource(struct rdt_data *data)
{
	...
	TEST_ASSERT_VAL("wrong L3 cbm_mask",     res->cbm_mask     == 0x1fffff);
	...
	TEST_ASSERT_VAL("wrong L2 cbm_mask",     res->cbm_mask     == 0x1fffff);
	TEST_ASSERT_VAL("wrong L2 min_cbm_bits", res->min_cbm_bits == 1);
...
";
	...
	TEST_ASSERT_VAL("failed to parse rdt data", !rdt_parse(&data, strdup(test)));

...
typedef struct perf_data_file *(get_file_cb_t)(struct perf_data *data);

static struct perf_data_file *get_file_single(struct perf_data *data)
{
	return perf_data__file(data);
}

static int session_write(char *path, get_file_cb_t file_cb)
{
	...
	TEST_ASSERT_VAL("failed to write header",
		!perf_session__write_header(session, evlist,
					    header->fd, false));
...
do {								\
	int i;							\
	for (i = 0; i < cnt; i++) {				\
		struct perf_data_file *file = file_cb(&data);	\
								\
		TEST_ASSERT_VAL(str,				\
...
static int test_generated_data(void)
{
	...
	err = session_write(file, get_file_single);
	if (!err)

			      ...
			      struct machine *machine __maybe_unused)
{
	...
	TEST_ASSERT_VAL("wrong cpus", perf_cpu_map__cpu(map, 0) == 1);
	TEST_ASSERT_VAL("wrong cpus", perf_cpu_map__cpu(map, 1) == 2);
	TEST_ASSERT_VAL("wrong cpus", perf_cpu_map__cpu(map, 2) == 3);

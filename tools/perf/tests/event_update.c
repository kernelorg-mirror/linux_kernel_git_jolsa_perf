			      ...
			      struct machine *machine __maybe_unused)
{
	...
	TEST_ASSERT_VAL("wrong cpus", cpu_map__nr(map) == 3);
	TEST_ASSERT_VAL("wrong cpus", cpu_map__cpu(map, 0) == 1);
	TEST_ASSERT_VAL("wrong cpus", cpu_map__cpu(map, 1) == 2);
	TEST_ASSERT_VAL("wrong cpus", cpu_map__cpu(map, 2) == 3);
	cpu_map__put(map);
	return 0;
}

...
static int bp_event(void *fn, int group_control, int group_fd, int period)
{
	...
	pe.group_control = group_control;
...
static int test_read(void)
{
	...
	gid_event1 = bp_event(bp_event1, 0, fd, 0);
	TEST_ASSERT_VAL("create event1", gid_event1 >= 0);

	gid_event2 = bp_event(bp_event2, 0, fd, 0);
...
static int test_mmap(void)
{
	...
	gid_event1 = bp_event(bp_event1, 0, fd, 1);
	TEST_ASSERT_VAL("create event1", gid_event1 >= 0);

	gid_event2 = bp_event(bp_event2, 0, fd, 1);
	TEST_ASSERT_VAL("create event2", gid_event2 >= 0);

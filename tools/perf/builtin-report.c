...
static void task__print_level(struct task *task, FILE *fp, int level)
{
	...
	fprintf(fp, "  %7d %7d %7d |%*s%s\n",
		thread->pid_, thread->tid, thread->ppid, level, "", thread__comm_str(thread));
...
static int task_print(struct report *rep, FILE *fp)
{
	...
	/* No locking needed, --task is single threaded command. */
...
int cmd_report(int argc, const char **argv)
{
	...
	OPT_BOOLEAN(0, "stat", &report.stat_mode, "dump stats"),
	OPT_BOOLEAN(0, "task", &report.task_mode, "dump tasks"),
...
repeat:
	...
	} else if (use_browser == 0 && !quiet && !report.task_mode) {

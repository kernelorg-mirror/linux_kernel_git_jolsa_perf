...
static int perf_stat_synthesize_config(bool is_pipe)
{
	...
	err = perf_event__synthesize_cpu_map(NULL, evsel_list->cpus,
					     process_synthesized_event,
					     NULL, PERF_STAT_CPU_MAP_TYPE__GLOBAL);
	if (err < 0) {
		pr_err("Couldn't synthesize thread map.\n");
		return err;
	}
	...
	if (aggr_map) {
		err = perf_event__synthesize_cpu_map(NULL, aggr_map,
						     process_synthesized_event,
						     NULL, PERF_STAT_CPU_MAP_TYPE__AGGR);
		if (err < 0) {
			pr_err("Couldn't synthesize thread map.\n");
			return err;
		}
			      ...
			      struct perf_session *session __maybe_unused)
{
	...
	perf_stat_init_aggr_mode(false);
	return 0;
}
			  ...
			  struct perf_session *session __maybe_unused)
{
	...
	switch (event->cpu_map.type) {
	case PERF_STAT_CPU_MAP_TYPE__GLOBAL:
		if (stat->cpus) {
		...
		stat->cpus = cpus;
		return set_maps(stat);
	case PERF_STAT_CPU_MAP_TYPE__AGGR:
		if (aggr_map) {
			pr_warning("Extra aggr map event, ignoring.\n");
			return 0;
		}
...
static int __cmd_report(int argc, const char **argv)
{
	...
	session = perf_session__new(&file, false, &perf_stat.tool);
	if (session == NULL)
		return -1;

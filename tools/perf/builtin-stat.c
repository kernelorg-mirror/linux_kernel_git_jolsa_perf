...
{
	if (stat_record.config.initial_delay)
		usleep(stat_record.config.initial_delay * USEC_PER_MSEC);
	 ...
	 */
	if (!target__none(&target) || stat_record.config.initial_delay)
...
static int __run_perf_stat(int argc, const char **argv, int run_idx)
{
	...
	evlist__for_each_entry(stat_record.evlist, counter) {
try_again:
		if (create_perf_stat_counter(counter, &stat_record.config, &target) < 0) {
...
static const struct option stat_options[] = {
	...
	OPT_BOOLEAN('i', "no-inherit", &stat_record.config.no_inherit,
	...
	OPT_BOOLEAN('c', "scale", &stat_record.config.scale, "scale/normalize counters"),
	...
	OPT_UINTEGER('D', "delay", &stat_record.config.initial_delay,
  ...
  { .type = PERF_TYPE_HW_CACHE,
	...
	if (transaction_run) {
		...
		if (metricgroup__has_metric("transaction")) {
			struct option opt = { .value = &stat_record.evlist };

			return metricgroup__parse_groups(&opt, "transaction",
							 &stat_record.config.metric_events);
		}
...
int cmd_stat(int argc, const char **argv)
{
	 ...
	 * stat sessions.
	 */
	stat_record.config.identifier = !(STAT_RECORD && perf_stat.data.is_pipe);

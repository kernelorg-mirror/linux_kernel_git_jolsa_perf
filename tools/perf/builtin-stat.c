...
static void nsec_printout(struct perf_stat_config *config,
			  int id, int nr, struct perf_evsel *evsel, double avg)
{
	...
	fmt_v = config->csv_output ? "%.6f%s" : "%18.6f%s";
	fmt_n = config->csv_output ? "%s" : "%-25s";

	aggr_printout(config, evsel, id, nr);

	scnprintf(name, sizeof(name), "%s%s",
		  perf_evsel__name(evsel), config->csv_output ? "" : " (msec)");

	fprintf(output, fmt_v, msecs, config->csv_sep);

	if (config->csv_output)
		fprintf(output, "%s%s", evsel->unit, config->csv_sep);
	else
		fprintf(output, "%-*s%s", config->unit_width, evsel->unit, config->csv_sep);
		     ...
		     struct runtime_stat *st)
{
	...
	if (config->metric_only)
		...
		nsec_printout(config, id, nr, counter, uval);
	else
			   ...
			   char *prefix, struct timespec *ts)
{
	...
	if ((num_print_interval == 0 && metric_only) || config->interval_clear)

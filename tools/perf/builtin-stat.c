				...
				int cpu)
{
	...
	else if (perf_evsel__is(counter, CYCLES_IN_TX))
		update_stats(&runtime_transaction_stats[ctx][cpu], count[0]);
	else if (perf_evsel__is(counter, TRANSACTION_START))
		update_stats(&runtime_transaction_stats[ctx][cpu], count[0]);
	else if (perf_evsel__is(counter, ELISION_START))
...
static void print_shadow_stats(FILE *out, struct perf_evsel *evsel, double avg, int cpu)
{
	...
	} else if (perf_evsel__is(evsel, CYCLES_IN_TX_CP)) {
		total = avg_stats(&runtime_cycles_stats[ctx][cpu]);
	...
	} else if (perf_evsel__is(evsel, TRANSACTION_START) &&
		   avg > 0 &&

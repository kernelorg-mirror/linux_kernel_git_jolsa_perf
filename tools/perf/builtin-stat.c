...
{
	return STAT_RECORD || evsel__attr(counter)->read_format & PERF_FORMAT_ID;
}
...
static int cpu_map__get_max(struct perf_cpu_map *map)
{
	...
	for (i = 0; i < perf_cpu_map__nr(map); i++) {
		if (perf_cpu_map__cpu(map, i) > max)
			max = perf_cpu_map__cpu(map, i);
	}
			       ...
			       aggr_get_id_t get_id, struct perf_cpu_map *map, int idx)
{
	...
	if (perf_cpu_map__cpu(config->cpus_aggr_map, cpu) == -1)
		perf_cpu_map__set(config->cpus_aggr_map, cpu, get_id(config, map, idx));

	return perf_cpu_map__cpu(config->cpus_aggr_map, cpu);
}
...
static int perf_stat_init_aggr_mode(void)
{
	...
	stat_config.cpus_aggr_map = perf_perf_cpu_map__empty_new(nr + 1);
...
{
	return perf_cpu_map__build_map(cpus, sockp, perf_env__get_socket, env);
}

static int perf_env__build_die_map(struct perf_env *env, struct perf_cpu_map *cpus,
				   struct perf_cpu_map **diep)
{
	return perf_cpu_map__build_map(cpus, diep, perf_env__get_die, env);
}

static int perf_env__build_core_map(struct perf_env *env, struct perf_cpu_map *cpus,
				    struct perf_cpu_map **corep)
{
	return perf_cpu_map__build_map(cpus, corep, perf_env__get_core, env);
}

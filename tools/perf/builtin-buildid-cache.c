...
static int cache_file__walk(walk_cb_t cb, void *data)
{
	struct rb_node *nd;
	int ret = 0;
...
static void date_snprintf(time_t t, char *buf, int sz)
{
	struct tm tm;
...
static int cache_files__process(FILE *out)
{
	...
	if (want_post_process())
		ret = cache_file__walk(cache_file__process, out);

	return ret;
}

static bool is_below_limit(const struct stat *st)
{
	bool below_limit = false;

	switch (cache_limit) {
	case CACHE_LIMIT__TIME:
		below_limit = st->st_atime < cache_limit__time;
		break;
	case CACHE_LIMIT__SIZE:
		below_limit = (u64) st->st_size < cache_limit__size;
		break;
...
{
	cache_file__walk(cache_file__remove, NULL);
...
static int setup_limit(char *limit)
{
	...
	for (i = 0; i < ARRAY_SIZE(suffix_time); i++) {
		...
		date_snprintf(cache_limit__time, buf, 100);
	...
	for (i = 0; i < ARRAY_SIZE(suffix_size); i++) {
		...
		size_snprintf(cache_limit__size, buf, 100);

...
static void print_rdt(struct perf_header *ph __maybe_unused,
		      int fd __maybe_unused, FILE *fp __maybe_unused)
{
	char *tmp, *line, *str = strdup(ph->env.rdt_str);
		       ...
		       void *data __maybe_unused)
{
	...
	ph->env.rdt_str = do_read_string(fd, ph);
	if (ph->env.rdt_str)
		ret = rdt_parse_map(&ph->env.rdt_data, ph->env.rdt_str);

...
event_update_event__alloc(size_t size, u64 type, u64 id)
{
					 ...
					 perf_event__handler_t process)
{
	...
	ev = event_update_event__alloc(size + 1, PERF_EVENT_UPDATE__UNIT, evsel->id[0]);
	if (ev == NULL)
		return -ENOMEM;
					  ...
					  perf_event__handler_t process)
{
	...
	ev = event_update_event__alloc(sizeof(*ev_data), PERF_EVENT_UPDATE__SCALE, evsel->id[0]);
	if (ev == NULL)
		return -ENOMEM;
					 ...
					 perf_event__handler_t process)
{
	...
	ev = event_update_event__alloc(len + 1, PERF_EVENT_UPDATE__NAME, evsel->id[0]);
	if (ev == NULL)
		return -ENOMEM;

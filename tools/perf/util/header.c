...
static int load_python_stack(struct feat_fd *ff, struct dso *dso)
{
	...
	while (offset < limit) {
		char buf[PATH_MAX];
		struct python_func *func = (struct python_func *) buf;
		struct symbol *sym;
		ssize_t len;

		if (readn(ff->fd, &func->header, sizeof(func->header)) != sizeof(func->header))
			return -1;

		len = func->header.size - sizeof(func->header);

		if (readn(ff->fd, (void *) &func->start, len) != len)
			return -1;

		sym = symbol__new(func->start, func->end - func->start,

			       ...
			       struct evsel *evsel)
{
	...
	int eu = evsel ? attr->exclude_user : 0;
	int ek = evsel ? attr->exclude_kernel : 0;
	int eh = evsel ? attr->exclude_hv : 0;
	int eH = evsel ? attr->exclude_host : 0;
	int eG = evsel ? attr->exclude_guest : 0;
	int eI = evsel ? attr->exclude_idle : 0;
	int precise = evsel ? attr->precise_ip : 0;
	int precise_max = 0;
	int sample_read = 0;
	int pinned = evsel ? attr->pinned : 0;
...
int parse_events__modifier_event(struct list_head *list, char *str, bool add)
{
	...
	__evlist__for_each_entry(list, evsel) {
		...
		attr->exclude_user   = mod.eu;
		attr->exclude_kernel = mod.ek;
		attr->exclude_hv     = mod.eh;
		attr->precise_ip     = mod.precise;
		attr->exclude_host   = mod.eH;
		attr->exclude_guest  = mod.eG;
		attr->exclude_idle   = mod.eI;
				   ...
				   const void *arg __maybe_unused)
{
	...
	if (evsel == NULL || evsel__attr(evsel)->type != PERF_TYPE_TRACEPOINT) {
		fprintf(stderr,
...
static bool is_event_supported(u8 type, unsigned config)
{
	...
	struct perf_thread_map *tmap = perf_thread_map__new_by_tid(0);
	...
	if (evsel) {
		...
		if (open_return == -EACCES) {
			 ...
			 *
			 */
			evsel__attr(evsel)->exclude_kernel = 1;

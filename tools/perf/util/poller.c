...
static int process_item(struct poller_item *item, short revents)
{
	bool has_data  = !!item->ops.data;
	bool has_error = !!item->ops.error;

	if (revents & POLLIN)
		return has_data  ? item->ops.data(item) : 0;
	else
		return has_error ? item->ops.error(item) : 0;
...
static int process(struct poller *p, int n)
{
	...
	for (i = 0; (i < p->n) && n; pfd++,i++) {
		...
		ret = process_item(item, pfd->revents);
		if (ret)
			return ret;
	}

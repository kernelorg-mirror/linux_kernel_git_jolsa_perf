				    ...
				    struct addr_location *al)
{
	...
	if (!event) {
		pr_debug("ug! no event found for type %" PRIu64, (u64) evsel__attr(evsel)->config);
		return;
	}
				       ...
				       struct evsel *evsel)
{
	...
	XPUSHs(sv_2mortal(newSVpvn((const char *)evsel__attr(evsel), sizeof(struct perf_event_attr))));

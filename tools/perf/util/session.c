...
static bool perf_session__has_comm_exec(struct perf_session *session)
{
	...
	evlist__for_each_entry(session->evlist, evsel) {
		if (evsel__attr(evsel)->comm_exec)
			return true;
	}
			...
			struct perf_sample *sample)
{
	...
	sample_type = evsel__attr(evsel)->sample_type;
	...
	if (sample_type & PERF_SAMPLE_READ)
		sample_read__printf(sample, evsel__attr(evsel)->read_format);
}
...
static void dump_read(struct evsel *evsel, union perf_event *event)
{
	...
	read_format = evsel__attr(evsel)->read_format;
...
{
	/* We know evsel != NULL. */
	u64 sample_type = evsel__attr(evsel)->sample_type;
			     ...
			     const char *cpu_list, unsigned long *cpu_bitmap)
{
	...
	for (i = 0; i < PERF_TYPE_MAX; ++i) {
		...
		if (!(evsel__attr(evsel)->sample_type & PERF_SAMPLE_CPU)) {

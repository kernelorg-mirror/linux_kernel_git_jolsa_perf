...
int rdt_parse(struct rdt_data *rdt, char *map)
{
	struct parse_data __data = {
		.rdt = rdt,
		.map = map,
	};
	struct parse_data *data = &__data;
	size_t size = strlen(map);
	int len, err = -1;

	data->tokens = data->curr = parse_json_map(map, size, &len);
	if (!data->tokens)
		return -1;
	...
	err = 0;
out:
	free_json(data->map, size, data->tokens);
	return err;
}

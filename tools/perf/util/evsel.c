				  ...
				  int thread, int err)
{
	...
	if (perf_thread_map__nr(threads) == 1)
		return false;
...
retry_open:
			...
			if (fd < 0) {
				...
				if (ignore_missing_thread(evsel, perf_cpu_map__nr(cpus), cpu, threads, thread, err)) {
					/*
...
int perf_evsel__store_ids(struct evsel *evsel, struct evlist *evlist)
{
	...
	if (perf_evsel__alloc_id(evsel, perf_cpu_map__nr(cpus), perf_thread_map__nr(threads)))
		return -ENOMEM;

...
{
	/*
	 * No need to check for dso->data.fd >= 0 here. We are
	 * called only from dso__data_close after this check.
	 */
...
static ssize_t mmaped_read(struct dso *dso, struct machine *machine,
			   u64 offset, u8 *data, ssize_t size)
{
	ssize_t rsize = -1;
	int fd;
			      ...
			      u64 offset, u8 *data, ssize_t size)
{
	...
	if (!dso->data.cached_read) {
		ret = mmaped_read(dso, machine, offset, data, size);
		dso->data.cached_read = true;
	}

	return ret ? ret : cached_read(dso, machine, offset, data, size);
}

	   ...
	   struct hist_entry *he)
{
	...
	for (node = 0; node < c2c.nodes_cnt; node++) {
		...
		case 1:
		{
			...
			if (c2c_hists->stats.rmt_hitm > 0)
				ret = scnprintf(hpp->buf, hpp->size, "%5.1f%% ", 100. * ((double)stats->rmt_hitm / (double) c2c_hists->stats.rmt_hitm));
			else
				ret = scnprintf(hpp->buf, hpp->size, "%6s ", "n/a");

			advance_hpp(hpp, ret);

			if (c2c_hists->stats.store > 0)
				ret = scnprintf(hpp->buf, hpp->size, "%5.1f%%}", 100. * ((double)stats->store / (double)c2c_hists->stats.store));
			else
				ret = scnprintf(hpp->buf, hpp->size, "%6s}", "n/a");

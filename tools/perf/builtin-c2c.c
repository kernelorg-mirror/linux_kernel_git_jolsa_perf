				...
				struct machine *machine)
{
	...
	if (!ret) {
		/*
		 * There's already warning about missing
		 * sample's cpu value. Let's account all
		 * to node 0 in the case without any other
		 * warning.
		 */
...
{
	int width = hists__col_len(hists, dim_symbol.se->se_width_idx);
...
static int srcline_width(struct hists *hists)
{
	int width = hists__col_len(hists, dim_srcline.se->se_width_idx);
		     ...
		     struct hists *hists __maybe_unused)
{
	...
	if (dim == &dim_symbol)
		return symbol_width(hists);
	if (dim == &dim_srcline)
		return srcline_width(hists);
			...
			case DISPLAY_RMT:
				if (c2c_he->stats.rmt_hitm > 0)
					ret = scnprintf(hpp->buf, hpp->size, "%5.1f%% ",
							100. * ((double)stats->rmt_hitm / (double) c2c_he->stats.rmt_hitm));
				else
					ret = scnprintf(hpp->buf, hpp->size, "%6s ", "n/a");
				break;
			case DISPLAY_LCL:
				if (c2c_he->stats.lcl_hitm > 0)
					ret = scnprintf(hpp->buf, hpp->size, "%5.1f%% ",
							100. * ((double)stats->lcl_hitm / (double) c2c_he->stats.lcl_hitm));
				else
					ret = scnprintf(hpp->buf, hpp->size, "%6s ", "n/a");
			default:
				break;
...
static struct c2c_dimension dim_percent_ldmiss = {
	.header		= HEADER_BOTH("%All","Ld Miss"),
	.name		= "percent_ldmiss",
			...
			struct hist_entry *he)
{
	...
	if (!len) {
		...
		if (dim == &dim_symbol)
			len = symbol_width(he->hists);
		if (dim == &dim_srcline)
			len = srcline_width(he->hists);
	}
...
static void perf_c2c__hists_fprintf(FILE *out)
{
	...
	fprintf(out, "=================================================\n");
	fprintf(out, "#\n");
	fprintf(out, "# (cacheline data groupping: %s)\n\n", c2c.cl_sort);
...
static int perf_c2c__report(int argc, const char **argv)
{
	...
	OPT_BOOLEAN(0, "full-symbols", &c2c.symbol_full,
		    "Display full lenght of symbols"),
	OPT_CALLBACK_DEFAULT('g', "call-graph", &callchain_param,
			     "print_type,threshold[,print_limit],order,sort_key[,branch],value",
			     callchain_help, &parse_callchain_opt,
			     callchain_default_opt),
	OPT_STRING(0, "display", &display, NULL, "lcl,rmt"),
	...
	c2c_hists__reinit(&c2c.hists,
			...
			"stores_l1hit,"
			"stores_l1miss",
			c2c.display == DISPLAY_RMT ? "rmt_hitm" : "lcl_hitm"
			);

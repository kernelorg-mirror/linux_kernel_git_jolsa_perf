...
static int resort_offset_cb(struct hist_entry *he)
{
	...
	if (c2c_hists) {
		c2c_hists__reinit(c2c_hists,
			...
			"pid,tid,iaddr,median,mean,stddev,symbol,dso",
			"rmt_hitm,lcl_hitm");
...
static int resort_cl_cb(struct hist_entry *he)
{
	...
	if (display && c2c_hists) {
		c2c_hists__reinit(c2c_hists,
				  "offset,cl_rmt_hitm,cl_lcl_hitm,cl_stores_l1hit,cl_stores_l1miss,daddr,dsymbol,node",
...
static int setup_callchain(struct perf_evlist *evlist)
{
	...
	if (!callchain_param.enabled && mode != CALLCHAIN_NONE) {

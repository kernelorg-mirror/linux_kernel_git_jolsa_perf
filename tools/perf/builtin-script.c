...
static int perf_session__check_output_opt(struct perf_session *session)
{
	...
	for (j = 0; j < OUTPUT_TYPE_MAX; ++j) {
		...
		set_print_ip_opts(evsel__attr(evsel));
	}
...
{
	switch (evsel__attr(evsel)->config) {
			...
			struct evlist **pevlist)
{
	...
	if (evsel__attr(evsel)->type >= PERF_TYPE_MAX &&
	    evsel__attr(evsel)->type != PERF_TYPE_SYNTH)
		return 0;

	evlist__for_each_entry(evlist, pos) {
		if (evsel__attr(pos)->type == evsel__attr(pos)->type &&
		    pos != evsel)
			return 0;
	}
	...
	if (evsel__attr(evsel)->sample_type)
			      ...
			      struct machine *machine)
{
	...
	if (!evsel__attr(evsel)->sample_id_all) {
		sample->cpu = 0;
		sample->time = 0;

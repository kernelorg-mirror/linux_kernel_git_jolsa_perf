#include <stdbool.h>
#include <stdio.h>
#include <wchar.h>
#include <sys/mman.h>
#include <sys/prctl.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <sys/types.h>
#include <unistd.h>
#include <sys/syscall.h>   /* For SYS_xxx definitions */
#include <linux/hash.h>
#include <linux/list.h>

#include "script-sample.h"

#include "Python.h"
#include <frameobject.h>

static bool  printonly;
static char *printfile;
static char *dir;
static int   verbose;

#define PR_TASK_PERF_EVENTS_DATA	48

typedef uint64_t u64;

struct mmap_stack {
	u64	cnt;
	u64	data[0];
};

static __thread struct mmap_stack *stack;
static __thread int fd_events;

#define HLIST_BITS 8
#define HLIST_SIZE (1 << HLIST_BITS)

static struct hlist_head heads[HLIST_SIZE];

struct hash_entry {
	struct hlist_node	 node;
	u64			 code;
};

static struct hash_entry* hash_find(void *code)
{
	struct hlist_head *head;
	struct hash_entry *h;
	int hash;

	hash = hash_64((u64) code, HLIST_BITS);
	head = &heads[hash];

	hlist_for_each_entry(h, head, node) {
		if (h->code == (u64) code)
			return h;
	}

	return NULL;
}

static int __store_event(PyCodeObject *co)
{
	wchar_t buf_name[100];
	void *start = (void *) PyBytes_AS_STRING(co->co_code);
	u64   size  = (u64)    PyBytes_GET_SIZE(co->co_code);
	struct python_func *func;
	char buf[PATH_MAX];
	int n = sizeof(*func);

	func = (struct python_func *) buf;

	PyUnicode_AsWideChar(co->co_name, buf_name, 100);

	n += snprintf(func->name, PATH_MAX - sizeof(*func),
		      "%ls", buf_name);

	n += 1;

	n = PERF_ALIGN(n, sizeof(u64));

	func->header.id	  = PYTHON_DUMP__FUNC;
	func->header.size = n;
	func->start	  = (u64) start;
	func->end	  = (u64) start + size;

	if (verbose > 0) {
		fprintf(stdout, "perfsample func : %p-%p %s\n",
			start, start + size, func->name);
	}

	return n == write(fd_events, func, n) ? 0 : -1;
}

static void store_event(PyCodeObject *co)
{
	if (__store_event(co))
		perror("event store failed");
}

static void hash_add(PyCodeObject *co)
{
	void *code = PyBytes_AS_STRING(co);
	struct hash_entry *h;
	int hash;

	if (hash_find(code))
		return;

	h = calloc(1, sizeof(*h));
	if (!h) {
		perror("failed: calloc");
		return;
	}

	h->code = (u64) code;

	hash = hash_64((u64) code, HLIST_BITS);
	hlist_add_head(&h->node, &heads[hash]);

	store_event(co);
}

static void stack_push(PyCodeObject *co)
{
	u64 cnt = stack->cnt; // ? stack->cnt + 1 : 0;

	stack->data[cnt] = (u64) PyBytes_AS_STRING(co->co_code);
	stack->cnt++;

	if (verbose > 1) {
		fprintf(stdout, "push %lu %p\n",
			stack->cnt, (void *) stack->data[cnt]);
	}

	hash_add(co);
}

static void stack_pop(void)
{
	if (stack->cnt)
		stack->cnt--;

	if (verbose > 1) {
		fprintf(stdout, "pop  %lu %p\n",
			stack->cnt, (void*) stack->data[stack->cnt]);
	}
}

static void stack_line(PyCodeObject *co, int lasti)
{
	if (!stack->cnt)
		stack_push(co);

	stack->data[stack->cnt] = (u64) PyBytes_AS_STRING(co->co_code) + lasti;

	if (verbose > 1) {
		fprintf(stdout, "line %lu %p\n",
			stack->cnt, (void*) stack->data[stack->cnt]);
	}
}

static inline pid_t gettid(void)
{
	return (pid_t) syscall(__NR_gettid);
}

static int config(void)
{
	char *v;

	printonly = !!getenv("PERFSAMPLE_PRINTONLY");
	printfile =   getenv("PERFSAMPLE_PRINTFILE");
	dir       =   getenv("PERFSAMPLE_DIR");
	v         =   getenv("PERFSAMPLE_VERBOSE");

	if (v)
		verbose = atoi(v);
	return 0;
}

static const char *what_str(unsigned int what)
{
	static const char *str[] = {
		"CALL",
		"EXCEPTION",
		"LINE",
		"RETURN",
		"C_CALL",
		"C_EXCEPTION",
		"C_RETURN",
	};

	if (what < 0 && what > 6)
		return "BUG";

	return str[what];
}

static int trace_print(PyFrameObject *frame, int what)
{
	static FILE *fp;
	PyObject *file, *name, *code;
	wchar_t buf_file[100], buf_name[100];
	int lineno, idx;
	char *func;

	if (!fp) {
		if (printfile) {
			fp = fopen(printfile, "w+");
			if (!fp) {
				perror("failed to open print file\n");
				return -1;
			}
		} else {
			fp = stderr;
		}
	}

	if (!frame || !frame->f_code)
		goto error;

	file = frame->f_code->co_filename;
	name = frame->f_code->co_name;
	code = frame->f_code->co_code;

	if (!file || !name || !code)
		goto error;

	PyUnicode_AsWideChar(file, buf_file, 100);
	PyUnicode_AsWideChar(name, buf_name, 100);

	lineno = PyCode_Addr2Line(frame->f_code, frame->f_lasti);
	func   = PyBytes_AS_STRING(code);
	idx    = frame->f_lasti != -1 ? frame->f_lasti : 0;

	fprintf(fp, "%-11s [%p+0x%05x] %ls %ls:%d\n", what_str(what), func, idx, buf_name, buf_file, lineno);
	fflush(NULL);
	return 0;

error:
	fprintf(fp, "%-11s ERROR\n", what_str(what));
	return 0;
}

static int config_thread(void)
{
	int protection = PROT_READ | PROT_WRITE;
	int visibility = MAP_SHARED;
	char path[PATH_MAX];
	int err, fd;

	snprintf(path, PATH_MAX, "%s/datauser-%d", dir, gettid());

	fd = open(path, O_RDWR|O_CREAT, 0644);
	if (fd < 0)
		return -1;

	if (ftruncate(fd, 4096))
		return -1;

	stack = mmap(NULL, 4096, protection, visibility, fd, 0);
	if ((void *) stack == MAP_FAILED) {
		perror("mmap failed");
		return -1;
	}

	err = prctl(PR_TASK_PERF_EVENTS_DATA, (unsigned long) stack, 0, 0, 0);
	if (err) {
		perror("prctl failed");
		return -1;
	}

	stack->cnt = 0;

	snprintf(path, PATH_MAX, "%s/events-%d", dir, gettid());

	fd = open(path, O_RDWR|O_CREAT, 0644);
	if (fd < 0)
		return -1;

	fd_events = fd;

	if (verbose > 0) {
		fprintf(stdout, "perfsample stack : %p\n", stack);
		fprintf(stdout, "perfsample dir   : %s\n", dir);
	}
	return 0;
}

static int trace_dir(PyFrameObject *frame, int what)
{
	if (!stack && config_thread()) {
		fprintf(stdout, "PICA PICA PICA\n");
		return 0;
	}

	switch (what) {
	case PyTrace_CALL:
		stack_push(frame->f_code);
		break;
	case PyTrace_RETURN:
		stack_pop();
		break;
	case PyTrace_LINE:
		stack_line(frame->f_code, frame->f_lasti);
		break;
	default:
		break;
	}
	return 0;
}

static int
trace(PyObject *self, PyFrameObject *frame, int what, PyObject *arg_unused)
{
	if (printonly)
		return trace_print(frame, what);
	if (dir)
		return trace_dir(frame, what);
	return 0;
}


#if PY_MAJOR_VERSION >= 3

static struct PyModuleDef moduledef = {
	PyModuleDef_HEAD_INIT,
	"perfsample",
	NULL,
	0,
	NULL,
	NULL,
	NULL,
	NULL,
	NULL
};

#define INITERROR return NULL

PyMODINIT_FUNC PyInit_perfsample(void)
#else
#define INITERROR return
void
initperfsample(void)
#endif
{
#if PY_MAJOR_VERSION >= 3
	PyObject *module = PyModule_Create(&moduledef);
#else
	PyObject *module = Py_InitModule("perfsample", myextension_methods);
#endif

	if (module == NULL)
		INITERROR;

	if (config())
		INITERROR;

	fprintf(stderr, "perfsample initialized\n");
	PyEval_SetTrace((Py_tracefunc)trace, (PyObject*)module);

#if PY_MAJOR_VERSION >= 3
	return module;
#endif
}

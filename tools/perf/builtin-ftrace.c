...
static int set_tracing_pid(struct perf_ftrace *ftrace)
{
	...
	for (i = 0; i < perf_thread_map__nr(ftrace->evlist->threads); i++) {
		scnprintf(buf, sizeof(buf), "%d",
			  perf_thread_map__pid(ftrace->evlist->threads, i));
...
static int set_tracing_cpumask(struct perf_cpu_map *cpumap)
{
	...
	last_cpu = perf_cpu_map__cpu(cpumap, perf_cpu_map__nr(cpumap) - 1);

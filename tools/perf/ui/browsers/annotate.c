	  ...
	  char *title, size_t sz)
{
	...
	return snprintf(title, sz, "%s  %s [Percent: %s]",
			sym->name, map->dso->long_name, pstr[percent_type]);
}
...
static int annotate_browser__show(struct ui_browser *browser, char *title, const char *help)
{
	...
	sym_title(sym, ms->map, ab->opts->percent_type, symbol_dso, sizeof(symbol_dso));
...
show_sup_ins:
		...
		case 'c':
			...
			continue;
		case 'p':
			switch_percent_type(browser->opts, false);
			hists__scnprintf_title(hists, title, sizeof(title));
			annotate_browser__show(&browser->b, title, help);
			continue;
		case 'b':
			switch_percent_type(browser->opts, true);
			hists__scnprintf_title(hists, title, sizeof(title));
			annotate_browser__show(&browser->b, title, help);
			continue;
		case K_LEFT:

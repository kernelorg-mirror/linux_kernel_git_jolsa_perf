...
static void hist_browser__init(struct hist_browser *browser,
			       struct hists *hists)
{
	struct perf_hpp_fmt *fmt;
				   ...
				   char *bf, size_t size)
{
	...
	scnprintf(bf, size,
		  "Shared Data Cache Line Table (%lu entries)",
		  hists->nr_entries);
	return 0;
}
				  ...
				  char *bf, size_t size)
{
	...
	scnprintf(bf, size,
		  "Cacheline 0x%lx - Shared Cache Line Distribution Pareto",
		  addr);
	return 0;
}

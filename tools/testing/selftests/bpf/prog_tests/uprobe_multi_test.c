...
#include "uprobe_multi_session_data.skel.h"
...
{
	struct uprobe_multi_session_data *skel = NULL;
	LIBBPF_OPTS(bpf_test_run_opts, topts);
	int err;

	skel = uprobe_multi_session_data__open_and_load();
	if (!ASSERT_OK_PTR(skel, "fentry_raw_skel_load"))
		goto cleanup;

	skel->bss->pid = getpid();

	err = uprobe_multi_session_data__attach(skel);
...
cleanup:
	uprobe_multi_session_data__destroy(skel);
}

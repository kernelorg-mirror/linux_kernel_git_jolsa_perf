...
{
	struct uprobe_multi_wrapper *skel = NULL;
	LIBBPF_OPTS(bpf_kprobe_multi_opts, opts);
	LIBBPF_OPTS(bpf_test_run_opts, topts);
	struct bpf_link *link = NULL;
	int err;

	skel = uprobe_multi_wrapper__open_and_load();
	if (!ASSERT_OK_PTR(skel, "fentry_raw_skel_load"))
		goto cleanup;

	skel->bss->pid = getpid();

	err =  uprobe_multi_wrapper__attach(skel);
	if (!ASSERT_OK(err, " uprobe_multi_wrapper__attach"))
		goto cleanup;

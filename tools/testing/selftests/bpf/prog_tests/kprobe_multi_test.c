...
#include "kprobe_multi_wrapper_data.skel.h"
...
{
	struct kprobe_multi_wrapper_data *skel = NULL;
	LIBBPF_OPTS(bpf_kprobe_multi_opts, opts);
	LIBBPF_OPTS(bpf_test_run_opts, topts);
	struct bpf_link *link = NULL;
	int err, prog_fd;

	skel = kprobe_multi_wrapper_data__open_and_load();
	if (!ASSERT_OK_PTR(skel, "fentry_raw_skel_load"))
		goto cleanup;

	skel->bss->pid = getpid();

	err =  kprobe_multi_wrapper_data__attach(skel);
	if (!ASSERT_OK(err, " kprobe_multi_wrapper__attach"))
		goto cleanup;
...
cleanup:
	bpf_link__destroy(link);
	kprobe_multi_wrapper_data__destroy(skel);
}

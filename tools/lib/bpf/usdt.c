					  ...
					  __u64 usdt_cookie)
{
	...
	struct bpf_link *multi_link = NULL;
	__u64 *cookies = NULL;
	...
	if (kernel_supports(NULL, FEAT_UPROBE_LINK)) {
	...
	for (i = 0; i < target_cnt; i++) {
		...
		if (kernel_supports(NULL, FEAT_UPROBE_LINK)) {
			...
			multi_link = bpf_program__attach_uprobe_opts(prog, pid, path,
								      target->rel_ip, &opts);
			err = libbpf_get_error(multi_link);
			if (err) {
			...
			link->uprobes[i].link = multi_link;
	...
	if (kernel_supports(NULL, FEAT_UPROBE_LINK)) {
		...
		opts_multi.offsets = offsets;
		opts_multi.ref_ctr_offsets = ref_ctr_offsets;
		opts_multi.cookies = cookies;

		multi_link = bpf_program__attach_uprobe_multi(prog, pid, path, NULL, &opts_multi);
		err = libbpf_get_error(multi_link);
		if (err) {

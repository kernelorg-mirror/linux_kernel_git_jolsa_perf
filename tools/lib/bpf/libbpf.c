				      ...
				      const struct bpf_uprobe_multi_opts *opts)
{
	...
	const unsigned long *offsets = NULL;
	const char **resolved_paths = NULL;
	struct bpf_link *link = NULL;
	char errmsg[STRERR_BUFSIZE];
	const char **paths, **syms;
	char full_path[PATH_MAX];
	int i, err, link_fd, prog_fd;
	const __u64 *cookies;
	bool retprobe;
	size_t cnt;
	...
	if (!binary_path && !func_pattern && !paths)
		return libbpf_err_ptr(-EINVAL);
	if (binary_path && paths)
		return libbpf_err_ptr(-EINVAL);
	if (func_pattern && syms)
		return libbpf_err_ptr(-EINVAL);
	if (func_pattern && offsets)
		return libbpf_err_ptr(-EINVAL);
	if (syms && offsets)
		return libbpf_err_ptr(-EINVAL);
	if (func_pattern && cnt)
...
static int match_single(Elf *elf, const char *binary_path, const char *sname,
...
static int elf_for_each_symbol(Elf *elf, const char *binary_path,
			       int (*fn)(Elf *, const char *, const char *, GElf_Sym *, void *),
			       void *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(sh_types); i++) {
		...
		for (idx = 0; idx < nr_syms; idx++) {
			...
			ret = fn(elf, binary_path, sname, &sym, data);
			if (ret < 0)
				goto out;
		}
	}
...
static long elf_find_func_offset(Elf *elf, const char *binary_path, const char *name)
{
	...
	err = elf_for_each_symbol(elf, binary_path, match_single, &data);
...
static int match_multi(Elf *elf, const char *binary_path, const char *sname,
		       GElf_Sym *sym, void *_data)
{
			     ...
			     const char **syms, unsigned long **poffsets)
{
	...
	err = elf_for_each_symbol(elf, binary_path, match_multi, &data);
	if (err)
		goto out;

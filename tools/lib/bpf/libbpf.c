...
collect_reloc_data(struct reloc_desc *desc, struct bpf_object *obj,
		   GElf_Sym *sym, GElf_Rel *rel __maybe_unused)
{
			   ...
			   Elf_Data *data, struct bpf_object *obj)
{
	...
	for (i = 0; i < nrels; i++) {
		...
		if (sym.st_shndx == maps_shndx) {
			err = collect_reloc_maps(desc, obj, &sym);
		} else if (prog->idx == text_shndx) {
			err = collect_reloc_data(desc, obj, &sym, &rel);
		} else {
...
static u64 bpf_interp__run(struct bpf_interp *interp, struct bpf_program *prog, int start)
{
	...
	regs[BPF_REG_1] = interp->args[BPF_INTERP__ARG1];
	regs[BPF_REG_2] = interp->args[BPF_INTERP__ARG2];
	regs[BPF_REG_3] = interp->args[BPF_INTERP__ARG3];
	regs[BPF_REG_4] = interp->args[BPF_INTERP__ARG4];
	regs[BPF_REG_5] = interp->args[BPF_INTERP__ARG5];
	...
	while ((size_t)(i - prog->insns) < prog->insns_cnt) {
		...
		case BPF_JMP | BPF_CALL:
			if (si & RELO_CALL_USER_UNRESOLVED) {
				char *name = prog->call_resolve[si & ~RELO_CALL_USER_UNRESOLVED];

				i->imm = si = interp->resolve_cb(interp, name);
				if (si == (u32) -1)
					return (u64) -1;
			}

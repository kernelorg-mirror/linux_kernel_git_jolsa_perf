...
static const struct bpf_sec_def section_defs[] = {
	...
	SEC_DEF("kprobe.multi/",	KPROBE,	BPF_TRACE_FPROBE, SEC_NONE, attach_kprobe),
	SEC_DEF("kretprobe.multi/",	KPROBE,	BPF_TRACE_FPROBE, SEC_NONE, attach_kprobe),
...
static int resolve_fprobe_cb(unsigned long long sym_addr, char sym_type,
			     const char *sym_name, void *ctx)
{
	struct fprobe_resolve *res = ctx;
	int err;
		   ...
		   const struct bpf_kprobe_opts *kopts)
{
	...
	err = libbpf_kallsyms_parse(resolve_fprobe_cb, &res);
	if (err)
		goto error;
	if (!res.cnt) {
		err = -ENOENT;
	...
	opts.fprobe.addrs = ptr_to_u64(res.addrs);
...
static struct bpf_link *attach_kprobe(const struct bpf_program *prog, long cookie)
{
	...
	if (str_has_pfx(prog->sec_name, "kprobe/")) {
		func_name = prog->sec_name + sizeof("kprobe/") - 1;
	} else if (str_has_pfx(prog->sec_name, "kretprobe/")) {
		func_name = prog->sec_name + sizeof("kprobe/") - 1;
		opts.retprobe = true;

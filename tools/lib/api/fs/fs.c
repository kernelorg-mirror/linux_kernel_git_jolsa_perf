...
static int
__filename__read_ull(const char *filename, unsigned long long *value,
		     int base)
{
	char line[64];
	int fd = open(filename, O_RDONLY), err = -1;
...
{
	return __filename__read_ull(filename, value, 0);
}
...
{
	return __filename__read_ull(filename, value, 16);
}

...
static struct du_cie* find_cie(u8 *addr)
{
	struct rb_node *n = cies.rb_node;

	while (n) {
		struct du_cie *cie = rb_entry(n, struct du_cie , frame.rb_node);
...
static int add_cie(struct du_cie *entry)
{
	struct rb_node **p;
	...
	while (*p != NULL) {
		parent = *p;
		cie = rb_entry(parent, struct du_cie, frame.rb_node);
...
static int add_fde(struct du_fde *entry)
{
	struct rb_node **p;
	struct rb_node *parent = NULL;
	struct du_fde *fde;
	...
	while (*p != NULL) {
		parent = *p;
		fde = rb_entry(parent, struct du_fde, frame.rb_node);
...
int walk_fdes(fde_cb_t cb)
{
	...
	while (next) {
		struct du_fde *fde = rb_entry(next, struct du_fde, frame.rb_node);
		int ret;

			...
			struct perf_event *event)
{
	...
	if (event->attr.user_data) {
		...
		if (ctx->user_data.state == PERF_USER_DATA_STATE_ENABLE) {
			ctx->user_data.state = PERF_USER_DATA_STATE_ON;
	       ...
	       struct user_data *ud)
{
	...
	callchain = get_perf_callchain(regs, 0, kernel, user, max_stack, crosstask, true);
			 ...
			 struct pt_regs *regs)
{
	...
	if (ud.allow && ud.type) {
		...
		if (!ctx->user_data.state)
			ctx->user_data.state = PERF_USER_DATA_STATE_ENABLE;
	}
}

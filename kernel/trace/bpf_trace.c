...
static u64 bpf_kprobe_multi_cookie(struct bpf_run_ctx *ctx)
{
	...
	if (WARN_ON_ONCE(!ctx))
		return 0;
	run_ctx = container_of(current->bpf_ctx, struct bpf_kprobe_multi_run_ctx, session_ctx.run_ctx);
	link = run_ctx->link;
	if (!link->cookies)
		return 0;
...
static void uprobe_session(struct bpf_uprobe *uprobe, unsigned int depth, int ret)
{
	if (ret)
...
uprobe_multi_link_handler(struct uprobe_consumer *con, struct pt_regs *regs)
{
	...
	if (is_uprobe_multi_session(prog)) {
		data = &uprobe->data[depth];
		data[0] = 0;
	}

	ret = uprobe_prog_run(uprobe, prog, instruction_pointer(regs), regs, false, (void *) data);

	if (is_uprobe_multi_session(prog)) {
		uprobe_session(uprobe, depth, ret);
		return 0;
	}
	return ret;
}
...
uprobe_multi_link_ret_handler(struct uprobe_consumer *con, unsigned long func, struct pt_regs *regs)
{
	...
	return uprobe_prog_run(uprobe, prog, func, regs, true, (void *) data);
}

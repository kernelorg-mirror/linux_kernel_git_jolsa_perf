...
static u64 bpf_kprobe_multi_cookie(struct bpf_run_ctx *ctx)
{
	...
	if (WARN_ON_ONCE(!ctx))
		return 0;
	run_ctx = container_of(current->bpf_ctx, struct bpf_kprobe_multi_run_ctx, wrapper_ctx.run_ctx);
	link = run_ctx->link;
	if (!link->cookies)
		return 0;
...
int bpf_kprobe_multi_link_attach(const union bpf_attr *attr, struct bpf_prog *prog)
{
	...
	if (flags & BPF_F_KPROBE_MULTI_WRAPPER)
...
__bpf_kfunc bool bpf_wrapper_is_return(void)
{
	...
	wrapper_ctx = container_of(current->bpf_ctx, struct bpf_wrapper_run_ctx, run_ctx);
	return wrapper_ctx->is_return;
}

__bpf_kfunc __u64* bpf_wrapper_cookie(void)
{
	struct bpf_wrapper_run_ctx *wrapper_ctx;

	wrapper_ctx = container_of(current->bpf_ctx, struct bpf_wrapper_run_ctx, run_ctx);
	return wrapper_ctx->data;
}

__bpf_kfunc_end_defs();

BTF_KFUNCS_START(kprobe_multi_kfunc_set_ids)
BTF_ID_FLAGS(func, bpf_wrapper_is_return)
BTF_ID_FLAGS(func, bpf_wrapper_cookie)

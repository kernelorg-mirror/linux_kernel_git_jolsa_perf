...
struct bpf_trampoline_ops {
	...
	int (*unregister_fentry)(struct bpf_trampoline *tr, void *old_addr, void *data);
	int (*modify_fentry)(struct bpf_trampoline *tr, void *old_addr, void *new_addr,
...
static int unregister_fentry(struct bpf_trampoline *tr, void *old_addr, void *data __maybe_unused)
{
	void *ip = tr->func.addr;
	int ret;

	if (tr->func.ftrace_managed)
		ret = unregister_ftrace_direct(&direct_ops, (long) ip, (long)old_addr, false);
	else
			 ...
			 bool lock_direct_mutex, void *data __maybe_unused)
{
	...
	if (tr->func.ftrace_managed) {
		if (lock_direct_mutex)
			ret = modify_ftrace_direct(&direct_ops, (long) ip, (long)new_addr);
		else
			ret = modify_ftrace_direct_nolock(&direct_ops, (long) ip, (long)new_addr);
	} else {
				     ...
				     struct bpf_trampoline_ops *ops, void *data)
{
	...
	if (total == 0) {
		err = ops->unregister_fentry(tr, tr->cur_image->image, data);
...
again:
	...
	if (tr->cur_image)
		...
		err = ops->modify_fentry(tr, tr->cur_image->image, im->image, lock_direct_mutex, data);
	else
		/* first time registering */
				      ...
				      void *data)
{
	...
	if (cnt >= BPF_MAX_TRAMP_NODES)
		return -E2BIG;
	if (!hlist_unhashed(&node->tramp_hlist))
		/* prog already linked */
		return -EBUSY;
	hlist_for_each_entry(node_existing, &tr->progs_hlist[kind], tramp_hlist) {
	...
	hlist_add_head(&node->tramp_hlist, &tr->progs_hlist[kind]);
	tr->progs_cnt[kind]++;
...
static void bpf_shim_tramp_link_release(struct bpf_link *link)
{
	...
	WARN_ON_ONCE(bpf_trampoline_unlink_prog(&shim_link->node, shim_link->trampoline, NULL));
	...
	for (kind = 0; kind < BPF_TRAMP_MAX; kind++) {
		hlist_for_each_entry(node, &tr->progs_hlist[kind], tramp_hlist) {
			...
			if (p->bpf_func == bpf_func)
				return container_of(node, struct bpf_shim_tramp_link, node);
		}
	}
				    ...
				    int cgroup_atype)
{
	...
	err = __bpf_trampoline_link_prog(&shim_link->node, tr, NULL, &trampoline_ops, NULL);
	if (err)
		goto err;

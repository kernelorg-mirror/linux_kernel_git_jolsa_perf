...
{
	struct btf_bitmap *bmap = NULL, *bmap_new = NULL;
	struct bpf_trampoline *tr_new;
	int err = -ENOMEM;

	bmap = btf_bitmap_funcs_alloc();
	...
	btf_bitmap_copy(bmap, tr->multi.bmap);
	...
	list_add_tail(&tr_new->rollback.list, rollback);

	tr->rollback.bmap = bmap;

...
static void bpf_struct_ops_link_dealloc(struct bpf_link *link)
{
	struct bpf_struct_ops_tramp_link *st_link =
		container_of(link, struct bpf_struct_ops_tramp_link, link);
				      ...
				      bool allow_alloc)
{
	...
	size = arch_bpf_trampoline_size(model, flags, tnodes, NULL);
	if (size <= 0)
		return size ? : -EFAULT;
					   ...
					   void *value, u64 flags)
{
	...
	for_each_member(i, t, member) {
		...
		bpf_link_init(&st_link->link, BPF_LINK_TYPE_STRUCT_OPS,
			      &bpf_struct_ops_link_lops, prog);
		st_map->links[i] = &st_link->link;

		trampoline_start = image_off;
		err = bpf_struct_ops_prepare_trampoline(tnodes, &st_link->node,
						&st_ops->func_models[i],

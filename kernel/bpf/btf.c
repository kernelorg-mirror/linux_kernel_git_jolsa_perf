...
static int btf_funcs_id(u32 id)
{
	u32 *func;
	...
	return id >= btf_vmlinux->funcs_size ? 0 : id;
}

u32 btf_id_from_funcs_id(u32 id)
{
	if (id >= btf_vmlinux->funcs_size)
		return 0;
	return btf_vmlinux->funcs_ids[id];
}
...
struct btf_bitmap *btf_bitmap_funcs_id(u32 id)
{
...
int btf_bitmap_funcs_resolve(struct btf_bitmap *bmap, unsigned long **paddrs)
{
	...
	for_each_set_bit(bit, bmap->bm, bmap->size) {
		id = btf_vmlinux->funcs_ids[bit];

		t = btf_type_by_id(btf_vmlinux, id);
		if (!t)
			goto out_free;
		name = btf_name_by_offset(btf_vmlinux, t->name_off);
		if (!name)
			goto out_free;
		rid[i].name = name;
		rid[i].id = id;

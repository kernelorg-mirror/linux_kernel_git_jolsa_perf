...
{
	struct pt *pt = this_cpu_ptr(&pt_ctx);
	u64 ctl = READ_ONCE(event->hw.cpu.config);
	...
	WRITE_ONCE(event->hw.cpu.config, ctl);
...
void intel_pt_handle_vmx(int on)
{
	...
	/* Turn PTs back on */
	if (!on && event)
		wrmsrl(MSR_IA32_RTIT_CTL, event->hw.cpu.config);
